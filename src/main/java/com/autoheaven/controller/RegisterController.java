...
import com.autoheaven.model.BillingAddress;
import com.autoheaven.model.Customer;
import com.autoheaven.model.ShippingAddress;
    ...
    public String registerCustomerPost(@Valid @ModelAttribute("customer") Customer customer, BindingResult result, Model model){
        ...
        for (int i=0; i<customerList.size(); i++){
            if (customer.getCustomerEmail().equals(customerList.get(i).getCustomerEmail())){
                model.addAttribute("emailMsg", "E-Mail already exists");

                return "registerCustomer";
            }

            if (customer.getCustomerUsername().equals(customerList.get(i).getCustomerUsername())){
